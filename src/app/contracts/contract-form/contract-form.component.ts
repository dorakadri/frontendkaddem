...
export class ContractFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(
      (params) => {
        ...
        if (this.id != null) {
          this.action = "update";
          this.contractApi.getContract(this.id).subscribe(
            (res) => {
              this.contract = res;
              ...
              this.contractForm
                ...
                .patchValue(this.formatDate(this.contract.dateDebutContrat));
              this.contractForm
                .get("dateFinContrat")
                .patchValue(this.formatDate(this.contract.dateFinContrat));
              this.contractForm
                .get("montantContrat")
                .setValue(this.contract.montantContrat);
              if (this.contract.archive) {
                const archivedInput = document.getElementById(
                  "ArchivedRadio"
                ) as HTMLInputElement | null;
                archivedInput.checked = true;
                this.contractForm.get("archive").setValue("true");
              } else {
                const nonArchivedInput = document.getElementById(
                  "NonArchivedRadio"
                ) as HTMLInputElement | null;
              ...
              if (this.contract.etudiant != null) {
                this.contractForm
                  .get("etudiant")
                  .get("nomE")
                  .setValue(this.contract.etudiant.nomE);
                this.contractForm
                  .get("etudiant")
                  .get("prenomE")
                  .setValue(this.contract.etudiant.prenomE);
                this.contractForm
                  .get("etudiant")
                  .get("date_naissance")
                  .patchValue(
                    this.formatDate(this.contract.etudiant.date_naissance)
                  );
                this.selectedOp = this.contract.etudiant.options;
              }
            },
            () => {
              console.log("error");
  ...
  onSubmit() {
    if (this.action == "new") {
      if (this.studentAction == "addUpdate") {
        this.contractApi.addContract(this.contractForm.value).subscribe(
          (res) => {
            console.log(res);
          },
          (error) => {
        ...
        this.contractApi.addContract(this.contractForm.value).subscribe(
          ...
          async () => {
            this.contractApi
              .affectContract(
                addedContract,
                this.selectedStudentName[0],
                this.selectedStudentName[1]
              )
              .subscribe(
                (res) => {
                  console.log(res);
      ...
      if (this.studentAction == "addUpdate") {
        this.contractApi.updateContract(this.contractForm.value).subscribe(
          (res) => {
            console.log(res);
          },
          (error) => {
  ...
  async alertAddWithSuccess() {
    const msg = await Swal.fire(
      "DONE",
      "Your contract added successfully!",
      "success"
    );
  }
  ...
  async alerUpdatetWithSuccess() {
    const msg = await Swal.fire(
      "DONE",
      "Your contract updated successfully!",
      "success"
    );
  }
